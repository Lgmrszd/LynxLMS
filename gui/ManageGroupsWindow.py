...
class ManageGroupsWindow(Window):
    def __init__(self, app):
        super().__init__(app)
        self.list = None
        self.click_search_button()
        self.app.el.register(self.deleted_listener, EventManager.Events.group_deleted, self)
        self.app.el.register(self.deleted_listener, EventManager.Events.group_changed, self)
        self.app.el.register(self.get_result, EventManager.Events.group_added, self)

    def deleted_listener(self, id):
        self.get_result()
    ...
    def cell_clicked_event(self, event):
        if self.list is not None and len(self.list) > event.row():
            self.app.open_window(GroupInfo, {"group": self.list[event.row()]})

...
class DocumentType (dc.BaseModel):
...
def doc_manager_init():
        dc.db.create_table(Document, safe=True)
        dc.db.create_table(DocumentType, safe=True)


def add_document(name, doctype, author, isBestseller, isReference, details):
        try:
                #TODO: details json checker
                Document.create(type = doctype, name = name, author = author, isBestseller = isBestseller, isReference = isReference, details = details)
        except ValueError:
                return(0)
        return(1)
